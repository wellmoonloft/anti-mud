...
func promptNick(c net.Conn, bufc *bufio.Reader) (nick *string) {
	...
	nick = new(string)
	for errorCount < 3 {
		io.WriteString(c, "请输入你的昵称 ")
		nickBytes, _, _ := bufc.ReadLine()
		*nick = string(nickBytes)
		// TODO: password
		if !isValidNick(*nick) {
			io.WriteString(c, "错误昵称，请重新输入！\n")
			errorCount++
			continue
		}
		...
		if err == nil {
			...
			if con, _ := player.isConnected(); con {
				io.WriteString(c, setFgBold(colorRed, fmt.Sprintf("%s 已经连接，请重试！\n", *nick)))
				errorCount++
				continue
			}

			io.WriteString(c, setFgBold(colorGreen, fmt.Sprintf("欢迎回来， %s!\n", *nick)))
			log.Printf("玩家 %+v 登陆\n", player)
			return
		}
		// Not found so create a new one.
		log.Printf("创建一个新玩家: %s\n", *nick)
		io.WriteString(c, "你看上去是个新玩家，请输入你的真实姓名。 ")
		realnameBytes, _, _ := bufc.ReadLine()
		realname = string(realnameBytes)
		log.Printf("增加真实姓名 %s for %s\n", realname, *nick)
		if player, err = players.add(*nick, realname); err == nil {
			io.WriteString(c, setFgBold(colorGreen, fmt.Sprintf("欢迎， %s!\n", *nick)))
			player.room = startRoomId
			return
		}
		errorLog.Printf("创建新玩家错误 %s %s: %+v\n", *nick, realname, err)
		errorCount++
	}
	return
}
